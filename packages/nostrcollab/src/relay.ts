...
import { Event, CountPayload, Filter, Pub, Sub, SubscriptionOptions } from "nostr-tools";
import {
	NostrCollabToken,
	KIND_COLLAB_TOKEN,
	KIND_COLLAB,
...
export class MockCollabRelay {
	...
	public sub(_filters: Filter[], _opts?: SubscriptionOptions): Sub {
		return {
			...
			unsub: () => {},
			on: <T extends keyof SubEvent, U extends SubEvent[T]>(_event: T, _listener: U) => {},
			off: <T extends keyof SubEvent, U extends SubEvent[T]>(_event: T, _listener: U) => {},
		};
	}
	...
	public get(filter: Filter, _opts?: SubscriptionOptions | undefined): Promise<Event | null> {
		...
		// 	});
			...
			// TODO: Find the appropriate kind to return here
			return Promise.resolve(null);
		}
			...
			if (request) {
				...
				try {
					...
					if (isNostrCreateCollabRequest(req)) {
						...
						// TODO: Find appropriate kind to return here
						return Promise.resolve(null);
					}
